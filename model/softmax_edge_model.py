...
当前公开默认档位为 ``HSSS-Softmax-block8``，对应与当前
``rtl/define.vh`` / ``rtl/softmax_top.v`` 对齐的位级语义配置；
``HSSS-Softmax-block4`` 作为 block size = 4 的 RTL 对齐配置。
...
PUBLIC_PROFILE = "HSSS-Softmax-block8"
PUBLIC_BLOCK8_PROFILE = PUBLIC_PROFILE
PUBLIC_BLOCK4_PROFILE = "HSSS-Softmax-block4"
DEFAULT_COMPARE_PROFILES: Tuple[str, ...] = (PUBLIC_PROFILE, "custom")
PROFILE_ALIASES = {
    PUBLIC_PROFILE.lower(): PUBLIC_PROFILE,
    "hsss-softmax": PUBLIC_PROFILE,
    "hsss": PUBLIC_PROFILE,
    PUBLIC_BLOCK4_PROFILE.lower(): PUBLIC_BLOCK4_PROFILE,
}


def canonical_precision_profile(profile: str) -> str:
    clean = profile.strip()
...
def get_precision_profile_overrides(profile: str) -> Dict[str, object]:
    ...
    设计原则：
    - ``HSSS-Softmax-block8``：当前 RTL 对齐的 2+7 描述符预设；
    - ``HSSS-Softmax-block4``：block size = 4 的 2+7 RTL 对齐预设；
    ...
    """
    profile = canonical_precision_profile(profile)
    if profile == "custom":
        return {}
    if profile == PUBLIC_BLOCK8_PROFILE:
        return {
            "lane_count": 8,
            "block_size": 8,
            "fx_int_width": 6,
    ...
    if profile == PUBLIC_BLOCK4_PROFILE:
        return {
            "lane_count": 4,
            "block_size": 4,
            "fx_int_width": 6,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--compare-precision",
        action="store_true",
        help="同时运行默认的 HSSS-Softmax-block8 与 custom，并打印对比结果",
    )
    parser.add_argument(
        "--compare-profiles",
        type=str,
        default="",
        help="逗号分隔的 profile 列表；为空时 --compare-precision 默认使用 HSSS-Softmax-block8,custom",
    )
    parser.add_argument(
        "--explain-output",
        action="store_true",
